...
def join_pair(rdd_pair, bcast_pair):
	if rdd_pair[0] in bcast_pair.value:
		avg_score = bcast_pair.value[rdd_pair[0]]
		return (rdd_pair[0], (rdd_pair[1], avg_score))
...
def main(inputs, output):
    ...
    avg_score_by_key_py = dict(avg_score_by_key.collect())  # first, turn RDD into a python object
    avg_score_by_key_py = sc.broadcast(avg_score_by_key_py)  # (subreddit, avg_score) pair is broadcast object now

    # key_comment_avg_pair = reddit_by_sub.join(avg_score_by_key_py.value)   # get (subrreddit, (text, avg_score)) pair (by key)
    # rdd.join(): joins two RDDs. But here we have an RDD and a broadcast object instead. 
    key_comment_avg_pair = reddit_by_sub.map(lambda x: join_pair(x, avg_score_by_key_py))  # when map() has more than 1 input
